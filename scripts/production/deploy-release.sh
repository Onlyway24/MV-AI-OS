...
while (($# > 0)); do
  case "$1" in
    ...
    *) die "usage: $0 --repo SSH_URL --branch feature/telegram-operator-console --commit FULL_SHA [--deploy-key ABS] [--known-hosts ABS] [--legacy-quiesce-receipt ABS --legacy-quiesce-signature ABS --legacy-rollback-receipt ABS] [--dry-run]" ;;
  esac
  shift
done
