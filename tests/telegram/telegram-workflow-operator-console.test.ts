...
describe("Telegram Workflow Operator Console", () => {
  it("promotes an approval-ready Mission through a separate confirmation and reports the durable checkpoints", async () => withDatabase(async (path) => {
    ...
    const inspected = await runtime.executeWorkflowCommand({
      actorId: "actor-local",
      commandId: "inspect-canonical-telegram-admission",
      contractVersion: "1",
      input: { instanceId: telegramWorkflowInstanceId("mission-draft-1") },
    ...
    expect(inspected.result).toMatchObject({
      ...
      input: {
        ...
        data: {
          ...
          objective: "Evaluate a bounded business opportunity for Fabio.",
        },
      },
      instanceId: telegramWorkflowInstanceId("mission-draft-1"),
      nonExecuting: true,
      status: "ACTIVE",
    });

    const admissionAudit = await runtime.executeWorkflowCommand({
      actorId: "actor-local",
      commandId: "inspect-canonical-telegram-admission-audit",
      contractVersion: "1",
      input: { correlationId: telegramWorkflowAdmissionId("mission-draft-1"), limit: 10 },
      operation: "INSPECT_AUDIT_EVENTS",
      workspaceId: "workspace-local",
    });
    expect(admissionAudit.result).toEqual([
      expect.objectContaining({
        action: "workflow.specification.admit",
        correlationId: telegramWorkflowAdmissionId("mission-draft-1"),
        eventType: "workflow.specification_admitted",
        outcome: "success",
      }),
    ]);
    expect(JSON.stringify(admissionAudit.result)).toMatch(
      /"inputFingerprint":"[a-f0-9]{64}"/u,
    );
...
function telegramWorkflowAdmissionId(reference: string): string { return `tg-wf-create-${createHash("sha256").update(reference, "utf8").digest("hex").slice(0, 24)}`; }
function telegramWorkflowInstanceId(reference: string): string { return `telegram-${createHash("sha256").update(reference, "utf8").digest("hex").slice(0, 24)}-workflow`; }
