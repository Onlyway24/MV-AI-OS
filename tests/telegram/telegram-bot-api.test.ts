...
import { FetchTelegramBotApiTransport, TelegramBotApiClient, type TelegramBotApiRequest, type TelegramBotApiTransport, type TelegramOperatorConfig } from "../../src/index.js";
...
describe("Telegram Bot API authorization", () => {
  ...
  it("enforces a client deadline and rejects redirects at the Telegram trust boundary", async () => {
    ...
    await expect(new FetchTelegramBotApiTransport(fetcher).request(apiRequest({ timeoutMs: 5 }))).rejects.toThrow("Telegram transport failed");
    expect(aborted).toBe(true);
    expect(observedInit).toMatchObject({ redirect: "error" });
  });
  it("rejects declared and streamed responses over the hard byte cap", async () => {
    let declaredCancelled = false;
    const declaredBody = new ReadableStream<Uint8Array>({ cancel: () => { declaredCancelled = true; }, start: (controller) => { controller.enqueue(new TextEncoder().encode("{}")); } });
    const declaredResponse = new Response(declaredBody, { headers: { "content-length": "1048577" } });
    await expect(new FetchTelegramBotApiTransport(fetchReturning(declaredResponse)).request(apiRequest())).rejects.toThrow("Telegram transport failed");
    expect(declaredCancelled).toBe(true);

    let streamedCancelled = false;
    const streamedBody = new ReadableStream<Uint8Array>({ cancel: () => { streamedCancelled = true; }, start: (controller) => { controller.enqueue(new Uint8Array(700_000)); controller.enqueue(new Uint8Array(400_000)); } });
    const streamedResponse = new Response(streamedBody, { headers: { "content-length": "2" } });
    await expect(new FetchTelegramBotApiTransport(fetchReturning(streamedResponse)).request(apiRequest())).rejects.toThrow("Telegram transport failed");
    expect(streamedCancelled).toBe(true);
  });
});

function apiRequest(overrides: Partial<TelegramBotApiRequest> = {}): TelegramBotApiRequest { return { body: {}, method: "getMe", timeoutMs: 1_000, token: "test-token", ...overrides }; }
function fetchReturning(response: Response): typeof fetch { return () => Promise.resolve(response); }
