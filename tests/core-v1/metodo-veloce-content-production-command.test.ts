...
describe("Metodo Veloce durable content production command", () => {
  ...
  it("migrates a Core V1 version 16 database before it creates the production queue", async () => withDatabase(async (path) => {
    ...
    legacy.exec("DROP TABLE feedback_metric_snapshots; DROP TABLE publication_kill_switches; DROP TABLE publication_plans; DROP TABLE evidence_records; DROP TABLE source_registry_entries; DROP TABLE production_runtime_jobs; DROP TABLE metodo_veloce_content_productions; DELETE FROM schema_migrations WHERE version IN (17, 18, 19); PRAGMA user_version = 16;");
    legacy.close();
