...
class InMemoryBusinessMissionRepository {
  ...
  public listApprovedByOwner(owner: { readonly actorId: string; readonly workspaceId: string }, limit: number): Promise<readonly BusinessMissionDossier[]> { if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100) throw new RepositoryValidationError("Business Mission list limit is invalid"); return Promise.resolve(Object.freeze([...this.state.businessMissions.values()].filter((record) => record.workspaceId === owner.workspaceId && record.actorId === owner.actorId && record.status === "APPROVED").sort((left, right) => right.updatedAt.localeCompare(left.updatedAt) || left.mission.missionId.localeCompare(right.mission.missionId)).slice(0, limit).map(cloneFrozen))); }
