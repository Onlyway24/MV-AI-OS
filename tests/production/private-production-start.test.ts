...
function validIntake(): PrivateProductionIntake {
  ...
  const workday: AgentCompanyWorkdayInput = {
    ...
    developer: { acceptanceChecks: ["lint", "typecheck", "test", "build"], filesInScope: ["src"], isolatedBranch: "codex/example-change", objective: "Preparare solo il piano tecnico interno senza merge o deploy" },
    maxBudgetCents: 0,
