...
describe("Production Closure CLI gates", () => {
  it("reports private security readiness only after the updated feature scan and every control proof", async () => {
    ...
    try {
      ...
      expect(ready).toMatchObject({
        ...
        status: "READY",
        targetBranch: "feature/telegram-operator-console",
      });
  ...
  it("emits every exact payment state and fails closed when durable proof is absent or tampered", async () => {
    ...
    try {
      ...
      const unsignedDeployment = {
        branch: "feature/telegram-operator-console" as const,
        commit: "b".repeat(40),
