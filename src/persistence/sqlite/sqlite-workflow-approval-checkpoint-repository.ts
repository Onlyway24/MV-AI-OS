...
import type { WorkflowApprovalCheckpoint } from "../../workflows/runtime/workflow-control-checkpoint.js";
import type { WorkflowApprovalCheckpointRepository } from "../../workflows/runtime/workflow-persistence.js";
...
export class SqliteWorkflowApprovalCheckpointRepository
  implements WorkflowApprovalCheckpointRepository
{
  ...
  public getById(evidenceId: string): Promise<WorkflowApprovalCheckpoint | undefined> {
    ...
    return Promise.resolve(withSqliteErrors("workflow_approval_checkpoint.get", () => {
  ...
  public insert(checkpoint: WorkflowApprovalCheckpoint): Promise<void> {
    assertActiveTransaction(this.#scope);
    const encoded = this.#codec.encodeWorkflowApprovalCheckpoint(checkpoint);
    withSqliteErrors("workflow_approval_checkpoint.insert", () => {
      ...
      try {
        this.#database.prepare(
          "INSERT INTO workflow_approval_checkpoints (evidence_id, definition_id, workflow_version, instance_id, instance_version, step_id, authority_actor_id, status, supersedes_evidence_id, recorded_at, record_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ).run(
          ...
          encoded.value.authorityActorId,
        ...
        if (isSqliteConstraintError(error)) {
          throw new RepositoryConflictError("Workflow approval checkpoint identity is invalid", {
            evidenceId: encoded.value.evidenceId,
          });
        }
        throw new SqliteRepositoryError(
          "SQLite workflow approval checkpoint insertion failed",
          "workflow_approval_checkpoint.insert",
        );
      }
    });
  ...
  public listBySnapshot(
    ...
    assertActiveTransaction(this.#scope);
    return Promise.resolve(withSqliteErrors("workflow_approval_checkpoint.list", () => {
      const rows = this.#database.prepare(
        "SELECT sequence, evidence_id, definition_id, workflow_version, instance_id, instance_version, step_id, authority_actor_id, status, supersedes_evidence_id, recorded_at, record_json FROM workflow_approval_checkpoints WHERE instance_id = ? AND instance_version = ? AND step_id = ? ORDER BY sequence ASC",
  ...
  #select(evidenceId: string): Readonly<Record<string, unknown>> | undefined {
    return this.#database.prepare(
      "SELECT sequence, evidence_id, definition_id, workflow_version, instance_id, instance_version, step_id, authority_actor_id, status, supersedes_evidence_id, recorded_at, record_json FROM workflow_approval_checkpoints WHERE evidence_id = ?",
    ).get(evidenceId);
  }

  #decode(row: Readonly<Record<string, unknown>>): WorkflowApprovalCheckpoint {
    const checkpoint = this.#codec.decodeWorkflowApprovalCheckpoint(
    ...
    if (
      ...
      readTextColumn(row, "authority_actor_id") !== checkpoint.authorityActorId ||
      ...
      throw new RepositoryValidationError(
        "SQLite workflow approval checkpoint columns do not match the record",
