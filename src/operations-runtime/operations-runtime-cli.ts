...
import { RepositoryBackedVentureOperationsBoundary } from "./repository-backed-venture-operations-boundary.js";
...
export async function runOperationsRuntimeCli(arguments_: readonly string[]): Promise<void> {
  ...
  try {
    ...
    const localWorkflows = createOperationsLocalWorkflowCallbacks({
      ...
      venture: new RepositoryBackedVentureOperationsBoundary({ actorId: config.runtime.actorId, clock, coreRepositories: repositories, repositories: ventureRepositories, workspaceId: config.runtime.workspaceId }),
      workspaceId: config.runtime.workspaceId,
    });
    ...
    const ventureRunner = ventureRepositories;
    await closeOperationsRuntimeResources({
      closeLock: () => lock.close(),
      closeRepositories: () => repositories.close(),
      ...(ventureRunner === undefined ? {} : { closeVentureRepositories: () => ventureRunner.close() }),
...
export async function closeOperationsRuntimeResources(input: Readonly<{
  ...
  readonly closeVentureRepositories?: () => Promise<void>;
}>): Promise<void> {
  const failures: unknown[] = [];
  const closeInOrder: readonly (() => Promise<void>)[] = [
    ...(input.closeService === undefined ? [] : [input.closeService]),
    ...(input.closeVentureRepositories === undefined ? [] : [input.closeVentureRepositories]),
