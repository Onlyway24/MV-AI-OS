...
export class MetodoVeloceContentProductionPackageValidator implements Validator<MetodoVeloceContentProductionPackage> {
  public validate(value: unknown): ValidationResult<MetodoVeloceContentProductionPackage> {
    ...
    const fields = value.assets === undefined ? ["approval", "contractVersion", "editorialPlan", "evidence", "externalActionsAllowed", "generatedAt", "metrics", "missionReference", "productionId", "quality", "risk", "status", "version"] : ["approval", "assets", "contractVersion", "editorialPlan", "evidence", "externalActionsAllowed", "generatedAt", "metrics", "missionReference", "productionId", "quality", "risk", "status", "version"];
    ...
    if (!keys(value, fields) || value.contractVersion !== METODO_VELOCE_CONTENT_PRODUCTION_CONTRACT_VERSION || value.version !== 1 || value.externalActionsAllowed !== false || !identifier(value.productionId) || !identifier(value.missionReference) || !timestamp(value.generatedAt) || !approval(value.approval) || !editorial(value.editorialPlan) || !evidenceSummary(value.evidence) || !quality(value.quality) || !risk(value.risk) || !metrics(value.metrics) || !["BLOCKED", "READY_FOR_FABIO_APPROVAL"].includes(value.status as string) || (value.status === "BLOCKED" && (value.assets !== undefined || approvalStatus !== "NOT_ELIGIBLE" || riskStatus !== "BLOCKED")) || (value.status === "READY_FOR_FABIO_APPROVAL" && (!assets(value.assets) || approvalStatus !== "PENDING_FABIO" || riskStatus !== "CLEAR"))) return invalid("Metodo Veloce content production package is invalid");
...
function carousel(value: unknown): boolean { return Array.isArray(value) && value.length === 7 && value.every((slide, index) => record(slide) && keys(slide, ["body", "slide", "title"]) && slide.slide === index + 1 && text(slide.title, 3, 100) && text(slide.body, 4, 500)); }
