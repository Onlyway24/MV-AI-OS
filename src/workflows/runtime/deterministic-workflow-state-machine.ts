...
const WF: Readonly<Record<WorkflowInstanceStatus, readonly WorkflowInstanceStatus[]>> = { ACTIVE:["PAUSED","COMPLETED","FAILED","CANCELLED"], PAUSED:["ACTIVE","CANCELLED"], COMPLETED:[], FAILED:["ACTIVE"], CANCELLED:[] };
const STEP: Readonly<Record<WorkflowStepInstanceStatus, readonly WorkflowStepInstanceStatus[]>> = { PENDING:["READY","CANCELLED"], READY:["AWAITING_RESULT","CANCELLED"], AWAITING_RESULT:["SUCCEEDED","FAILED","CANCELLED"], SUCCEEDED:[], FAILED:["READY"], CANCELLED:[] };
...
export class DeterministicWorkflowStateMachine {
  ...
  public apply(instance: WorkflowInstance, command: WorkflowCommand): WorkflowTransitionResult {
    ...
    if(instance.status==="CANCELLED"||instance.status==="COMPLETED"||(instance.status==="FAILED"&&command.kind!=="RETRY_STEP")) throw new WorkflowStateError("terminal workflow cannot transition","terminal_workflow");
