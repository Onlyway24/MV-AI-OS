...
export interface WorkflowAgentInvocationFailure {
  readonly code: "AGENT_EXECUTION_FAILED" | "AGENT_RESULT_INVALID";
...
export class WorkflowAgentInvocationReceiptValidator implements Validator<WorkflowAgentInvocationReceipt> {
  public validate(value: unknown): ValidationResult<WorkflowAgentInvocationReceipt> {
    ...
    if (value.contractVersion !== "1" || ids.some((key) => !safeId(value[key])) || !Number.isSafeInteger(value.reservedInstanceVersion) || (value.reservedInstanceVersion as number) < 1 || !Array.isArray(value.capabilityIds) || value.capabilityIds.some((id) => !safeId(id)) || !["RESERVED", "COMPLETED", "FAILED"].includes(value.status as string) || !timestamp(value.reservedAt) || value.externalEffectsAllowed !== false) return invalid("workflow agent invocation receipt is invalid");
