...
export class RepositoryBackedWorkflowOperatorReportService {
  ...
  public create(request: WorkflowOperatorReportRequest): Promise<WorkflowOperatorReport> {
    ...
    return this.repositories.transaction(async ({ workflows }) => {
      ...
      const workflowEvents = await workflows.events.listByInstanceId(instance.instanceId, valid.maxItems);
      const invocations = await workflows.agentInvocations.listByInstanceId(instance.instanceId, valid.maxItems);
      const stepEvidence = await Promise.all(definition.steps.map(async (step) => ({
        approvals: await workflows.approvals.listBySnapshot(instance.instanceId, instance.version, step.stepId),
        guardians: await workflows.guardians.listBySnapshot(instance.instanceId, instance.version, step.stepId),
        lifecycle: await workflows.lifecycleRecords.listByStep(instance.instanceId, step.stepId),
        step,
        state: instance.steps.find(({ stepId }) => stepId === step.stepId),
      })));
      ...
      const controls = stepEvidence.map((entry) => controlState(entry.step, entry.approvals.at(-1), entry.guardians));
      ...
      const report: WorkflowOperatorReport = {
        ...
        nextAction: nextAction(instance.status, instance.version, controls, readySteps, failedSteps, retry, actionableInvocations),
        nonExecuting: true,
        ...
        truncated: [completedSteps, readySteps, blockedSteps, pendingSteps, failedSteps, rawRisks].some((items) => items.length > valid.maxItems),
...
function controlState(step: WorkflowStepDefinition, approval: { readonly evidenceId: string; readonly status: string } | undefined, guardians: readonly WorkflowGuardianCheckpoint[]): ControlState { const latestByDomain = new Map<string, WorkflowGuardianCheckpoint>(); for (const guardian of guardians) latestByDomain.set(guardian.domain, guardian); const current = [...latestByDomain.values()]; const blocked = current.find(({ status }) => status === "BLOCKED"); const incomplete = current.find(({ status }) => status !== "CLEAR"); const representative = blocked ?? incomplete ?? current.at(-1); const guardianState: ControlState["guardian"] = !step.guardianRequired ? { remediationRequired: false, status: "NOT_REQUIRED" } : blocked !== undefined ? { domain: blocked.domain, evidenceId: blocked.evidenceId, remediationRequired: true, status: "BLOCKED" } : current.length > 0 && incomplete === undefined ? { ...(representative === undefined ? {} : { domain: representative.domain, evidenceId: representative.evidenceId }), remediationRequired: false, status: "CLEAR" } : { ...(representative === undefined ? {} : { domain: representative.domain, evidenceId: representative.evidenceId }), remediationRequired: true, status: "REQUIRED" }; return { stepId: step.stepId, approval: !step.approvalRequired ? { status: "NOT_REQUIRED" } : approval?.status === "APPROVED" ? { evidenceId: approval.evidenceId, status: "APPROVED" } : { ...(approval === undefined ? {} : { evidenceId: approval.evidenceId }), status: "REQUIRED" }, guardian: guardianState }; }
function stepReport(state: WorkflowStepInstance, definition: WorkflowStepDefinition, control: ControlState): WorkflowOperatorStepReport { const reasons: string[] = []; if (definition.approvalRequired && control.approval.status !== "APPROVED") reasons.push("Fabio approval required"); if (definition.guardianRequired && control.guardian.status !== "CLEAR") reasons.push(control.guardian.status === "BLOCKED" ? `Guardian ${control.guardian.domain ?? "review"} requires remediation` : "Guardian decision required"); if (state.blockers.some(({ code }) => code === "DEPENDENCY_INCOMPLETE")) reasons.push("Dependency incomplete"); if (state.status === "AWAITING_RESULT") reasons.push("Agent result is awaiting explicit review or timeout evaluation"); return freeze({ reasons: Object.freeze(reasons), status: state.status, stepId: state.stepId }); }
