...
const TERMINAL_WORKFLOW_STATUSES = new Set<WorkflowOperatorReport["overallStatus"]>(["CANCELLED", "COMPLETED", "FAILED"]);
const TERMINAL_STEP_STATUSES = new Set<WorkflowStepInstance["status"]>(["CANCELLED", "FAILED", "SUCCEEDED"]);
...
export class RepositoryBackedWorkflowOperatorReportService {
  ...
  public create(request: WorkflowOperatorReportRequest): Promise<WorkflowOperatorReport> {
    ...
    return this.repositories.transaction(async ({ workflows }) => {
      ...
      const controls = stepEvidence.map((entry) => controlState(entry.step, entry.approvals.at(-1), entry.guardians, entry.state?.status, instance.status));
      ...
      const workflowTerminal = isTerminalWorkflowStatus(instance.status);
      const readySteps = workflowTerminal ? [] : stepReports.filter(({ status, reasons }) => status === "READY" && reasons.length === 0);
      const blockedSteps = workflowTerminal ? [] : stepReports.filter(({ reasons, status }) => reasons.length > 0 && !isTerminalStepStatus(status));
      const pendingSteps = workflowTerminal ? [] : stepReports.filter(({ status, reasons }) => status === "PENDING" && reasons.length === 0);
      const lifecycle = stepEvidence.flatMap((entry) => entry.lifecycle);
      const retry = retryState(lifecycle, failedSteps[0]?.stepId);
      const rawRisks = instance.status === "FAILED"
        ? failedSteps.map(({ stepId }) => `${stepId}: failed`)
        : [...blockedSteps.flatMap(({ stepId, reasons }) => reasons.map((reason) => `${stepId}: ${reason}`))];
...
function stepReport(state: WorkflowStepInstance, definition: WorkflowStepDefinition, control: ControlState): WorkflowOperatorStepReport { if (control.terminal) return freeze({ reasons: Object.freeze([]), status: state.status, stepId: state.stepId }); const reasons: string[] = []; if (definition.approvalRequired && control.approval.status === "REQUIRED") reasons.push("Fabio approval required"); if (control.guardian.status !== "CLEAR" && control.guardian.status !== "NOT_REQUIRED") reasons.push(control.guardian.status === "BLOCKED" ? `Guardian ${control.guardian.domain ?? "review"} requires remediation` : `Guardian ${control.guardian.domain ?? "review"} decision required`); if (state.blockers.some(({ code }) => code === "DEPENDENCY_INCOMPLETE")) reasons.push("Dependency incomplete"); if (state.status === "AWAITING_RESULT") reasons.push("Agent result is awaiting explicit review or timeout evaluation"); return freeze({ reasons: Object.freeze(reasons), status: state.status, stepId: state.stepId }); }
function isTerminalWorkflowStatus(status: WorkflowOperatorReport["overallStatus"]): boolean { return TERMINAL_WORKFLOW_STATUSES.has(status); }
function isTerminalStepStatus(status: WorkflowStepInstance["status"]): boolean { return TERMINAL_STEP_STATUSES.has(status); }
function hasTerminalReportPrecedence(workflowStatus: WorkflowOperatorReport["overallStatus"], stepStatus: WorkflowStepInstance["status"] | undefined): boolean { return isTerminalWorkflowStatus(workflowStatus) || (stepStatus !== undefined && isTerminalStepStatus(stepStatus)); }
