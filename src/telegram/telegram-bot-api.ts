...
export class TelegramBotApiClient {
  ...
  public async deliver(intent: TelegramOutboundMessageIntent): Promise<void> {
    ...
    await this.#request("sendMessage", { chat_id: valid.value.chatId, text: valid.value.text });
  }
  public async setCommands(): Promise<void> { await this.#request("setMyCommands", { commands: ["start", "help", "status", "mission", "workflows", "report", "settings", "cancel_action", "stop"].map((command) => ({ command, description: commandDescription(command) })) }); }
  public normalize(raw: unknown): TelegramUpdateAuthorization {
    ...
    const payload = update.type === "callback_query" || (kind === "MISSION_DRAFT" && !text.startsWith("/") ) ? text : undefined;
...
function actionKind(value: string, callback: boolean): TelegramOperatorAction["kind"] | undefined { const command = value.trim().split(/\s+/u, 1)[0]?.toLowerCase(); const commands: Readonly<Record<string, TelegramOperatorAction["kind"]>> = { "/cancel_action": "CANCEL_ACTION", "/help": "HELP", "/mission": "MISSION_DRAFT", "/report": "REPORT", "/settings": "SETTINGS", "/start": "START", "/status": "STATUS", "/stop": "STOP", "/workflows": "WORKFLOWS" }; if (callback) return /^cb_[A-Za-z0-9_-]{16,128}$/u.test(value) ? "STATUS" : undefined; return command === undefined ? undefined : commands[command] ?? (value.startsWith("/") ? undefined : "MISSION_DRAFT"); }
function botResults(value: unknown): readonly unknown[] { return record(value) && Array.isArray(value.result) ? value.result : []; }
function commandDescription(command: string): string { return ({ cancel_action: "Annulla azione", help: "Aiuto", mission: "Nuova missione", report: "Report workflow", settings: "Impostazioni", start: "Avvia", status: "Stato", stop: "Arresta interazione", workflows: "Workflow" } as Record<string, string>)[command] ?? ""; }
