...
export class TelegramBotApiClient {
  ...
  public async setCommands(): Promise<void> { await this.#request("setMyCommands", { commands: ["start", "help", "status", "mission", "workflow", "workflows", "report", "productions", "production", "cancel_action", "stop", "developer"].map((command) => ({ command, description: commandDescription(command) })) }, "BOOTSTRAP"); }
  public normalize(raw: unknown): TelegramUpdateAuthorization {
    ...
    const payload = update.type === "callback_query" || ["CONTENT_PRODUCTION", "MISSION_DRAFT", "REPORT", "WORKFLOW"].includes(kind) ? text : undefined;
