...
export interface TelegramBotApiRequest { readonly method: "answerCallbackQuery" | "deleteWebhook" | "editMessageText" | "getMe" | "getUpdates" | "sendMessage" | "setMyCommands"; readonly token: string; readonly body: Readonly<Record<string, unknown>>; readonly timeoutMs: number; }
...
export class TelegramBotApiClient {
  ...
  async #request(method: TelegramBotApiRequest["method"], body: Readonly<Record<string, unknown>>, operation: "BOOTSTRAP" | "IDENTITY" | "OUTBOUND" | "POLLING"): Promise<unknown> {
    try {
      const timeoutMs = method === "getUpdates" ? (this.config.polling.timeoutSeconds + 5) * 1_000 : 15_000;
      const response = await this.transport.request({ body, method, timeoutMs, token: this.token });
...
export class FetchTelegramBotApiTransport implements TelegramBotApiTransport {
  ...
  public async request(request: TelegramBotApiRequest): Promise<unknown> {
    ...
    try {
      const response = await this.fetcher(`https://api.telegram.org/bot${request.token}/${request.method}`, { body: JSON.stringify(request.body), headers: { "content-type": "application/json" }, method: "POST", redirect: "error", signal: controller.signal });
...
const MAX_TELEGRAM_RESPONSE_BYTES = 1_048_576;

async function readBoundedJsonResponse(response: Response): Promise<unknown> {
  const declaredLength = response.headers.get("content-length")?.trim();
  if (declaredLength !== undefined && (/^\d+$/u.test(declaredLength) ? Number(declaredLength) > MAX_TELEGRAM_RESPONSE_BYTES : false)) {
    await cancelResponseBody(response.body);
    throw new Error("Telegram response exceeds the byte limit");
  }
  const body = response.body as ReadableStream<Uint8Array> | null;
  const reader = body?.getReader();
  if (reader === undefined) throw new Error("Telegram response body is unavailable");
  ...
  try {
    for (;;) {
      ...
      if (done) break;
      if (value.byteLength > MAX_TELEGRAM_RESPONSE_BYTES - totalBytes) {
  ...
  return JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(Buffer.concat(chunks, totalBytes))) as unknown;
}

async function cancelResponseBody(body: ReadableStream<Uint8Array> | null): Promise<void> { try { await body?.cancel(); } catch { return; } }
async function cancelResponseReader(reader: ReadableStreamDefaultReader<Uint8Array>): Promise<void> { try { await reader.cancel(); } catch { return; } }
