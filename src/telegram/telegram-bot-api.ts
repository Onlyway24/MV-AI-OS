...
export class TelegramBotApiClient {
  ...
  public async setCommands(): Promise<void> { await this.#request("setMyCommands", { commands: ["start", "help", "status", "mission", "workflow", "workflows", "report", "productions", "production", "evidencepack", "cancel_action", "stop", "developer"].map((command) => ({ command, description: commandDescription(command) })) }, "BOOTSTRAP"); }
  public normalize(raw: unknown): TelegramUpdateAuthorization {
    ...
    const payload = update.type === "callback_query" || ["CONTENT_PRODUCTION", "EVIDENCE_PACK", "MISSION_DRAFT", "REPORT", "WORKFLOW"].includes(kind) ? text : undefined;
...
function actionKind(value: string, callback: boolean): TelegramOperatorAction["kind"] | undefined { const command = value.trim().split(/\s+/u, 1)[0]?.toLowerCase(); const commands: Readonly<Record<string, TelegramOperatorAction["kind"]>> = { "/cancel_action": "CANCEL_ACTION", "/developer": "DEVELOPER", "/evidencepack": "EVIDENCE_PACK", "/help": "HELP", "/mission": "MISSION_DRAFT", "/production": "CONTENT_PRODUCTION", "/productions": "CONTENT_QUEUE", "/report": "REPORT", "/settings": "SETTINGS", "/start": "START", "/status": "STATUS", "/stop": "STOP", "/workflow": "WORKFLOW", "/workflows": "WORKFLOWS" }; if (callback) return /^cb_[A-Za-z0-9_-]{16,128}$/u.test(value) ? "WORKFLOW_CREATE" : undefined; return command === undefined ? undefined : commands[command] ?? (value.startsWith("/") ? undefined : "MISSION_DRAFT"); }
function botResults(value: unknown): readonly unknown[] { return record(value) && Array.isArray(value.result) ? value.result : []; }
function commandDescription(command: string): string { return ({ cancel_action: "Annulla azione", developer: "Sviluppatore", evidencepack: "Apri Evidence Pack", help: "Aiuto", mission: "Pianifica Missione", production: "Apri contenuto", productions: "Coda contenuti", report: "Report Workflow", start: "Avvia", status: "Stato", stop: "Arresta interazione", workflow: "Crea Workflow", workflows: "Guida Workflow" } as Record<string, string>)[command] ?? ""; }
