...
export class TelegramWorkflowOperatorConsole {
  ...
  async #contentProduction(identity: string, command: string): Promise<TelegramOutboundMessageIntent> {
    ...
    try {
      ...
      const callback = content.status === "PENDING_FABIO_APPROVAL" && content.evidencePack !== undefined ? this.input.state.issueCallback(identity, "CONTENT_REVIEW_APPROVE", this.input.confirmationRetentionSeconds, content.productionId, String(content.version)) : undefined;
...
function contentRecord(value: unknown): { readonly evidencePack?: { readonly fingerprint: string; readonly minFreshnessExpiresAt: string; readonly packId: string }; readonly productionId: string; readonly status: string; readonly version: number; readonly package: { readonly editorialPlan?: { readonly selectedIdea?: string }; readonly evidence?: { readonly items?: readonly unknown[] }; readonly risk?: { readonly status?: string } } } | undefined { return record(value) && typeof value.productionId === "string" && typeof value.status === "string" && Number.isSafeInteger(value.version) && record(value.package) && (value.evidencePack === undefined || (record(value.evidencePack) && typeof value.evidencePack.packId === "string" && typeof value.evidencePack.fingerprint === "string" && typeof value.evidencePack.minFreshnessExpiresAt === "string")) ? value as unknown as { readonly evidencePack?: { readonly fingerprint: string; readonly minFreshnessExpiresAt: string; readonly packId: string }; readonly productionId: string; readonly status: string; readonly version: number; readonly package: { readonly editorialPlan?: { readonly selectedIdea?: string }; readonly evidence?: { readonly items?: readonly unknown[] }; readonly risk?: { readonly status?: string } } } : undefined; }
...
function formatContent(content: NonNullable<ReturnType<typeof contentRecord>>): string { const idea = content.package.editorialPlan?.selectedIdea ?? "Direzione editoriale disponibile"; const risk = content.package.risk?.status ?? "non disponibile"; const evidenceCount = content.package.evidence?.items?.length ?? 0; const provenance = content.evidencePack === undefined ? "Provenienza: contenuto legacy senza Evidence Pack; non approvabile da Telegram." : `Evidence Pack: ${content.evidencePack.packId} · valido fino al ${content.evidencePack.minFreshnessExpiresAt}\nConsulta: /evidencepack ${content.evidencePack.packId}`; return [`Centro Produzione — ${content.productionId}`, `Stato: ${content.status} · versione ${String(content.version)}`, `Idea: ${idea}`, `Evidenze: ${String(evidenceCount)}`, provenance, `Rischio claim: ${risk}`, "", "Il contenuto resta preparation-only: nessuna pubblicazione è stata eseguita."].join("\n"); }
