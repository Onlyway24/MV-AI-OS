...
const WORKFLOW_VERSION = CORE_V1_CONTENT_DIRECTION_WORKFLOW_VERSION;
...
export class TelegramWorkflowOperatorConsole {
  ...
  async #createWorkflow(reference: string | undefined, version: string | undefined): Promise<TelegramOutboundMessageIntent> {
    ...
    const admissionId = commandId("create", reference);
    const response = await this.input.runtime.executeWorkflowCommand({
      ...
      input: {
        request: {
          ...
          input: {
            ...
            objective: source.draft.objective,
          },
          instanceId: workflowIds(reference).instanceId,
          nonExecuting: true,
    ...
    const created = record(response.result) && response.result.outcome === "ADMITTED" && !response.replayed;
...
function workflowIds(reference: string): { readonly instanceId: string } {
  const suffix = createHash("sha256").update(reference, "utf8").digest("hex").slice(0, 24);
  return { instanceId: `telegram-${suffix}-workflow` };
}
