...
The authoritative checkout is the repository root from which the validated release
commit is built. Alternate or shadow checkouts are outside operational and release
scope unless they resolve to that same Git worktree and exact commit.
...
- The complete 19-job catalog has `maxCostCents: 0` and
...
The scheduler provisions 19 idempotent schedules: the eleven system, workday, social,
evidence, approval, production, cost, backup, stale-task, daily-report and security
schedules plus opportunity scan, venture evidence refresh, experiment review, venture
stale check, portfolio daily/weekly review, capital-allocation review and venture
kill/scale review. The CLI composes real local callbacks for Founder Workday and Daily
Operating Brief. The worker also composes the canonical Venture Holding service and
its identity-scoped SQLite repository for all eight Venture jobs. Those jobs perform
bounded internal materialization/inspection; missing Founder policy, portfolio
coverage, registered evidence or real observations remains a precise durable
blocker, never simulated success. Backup success is accepted only from the explicit verification
boundary; a missing callback fails honestly.
...
Schema v32 validates the complete response locally and persists an opaque outbound
...
a persisted approval fingerprint. They also cover production composition of all eight
Venture schedules through the durable Venture adapter with zero external effects and
truthful policy/evidence/observation blockers. This is implementation evidence, not proof that a
