# Telegram Operator Console — integrated guide
...
Schema v32 makes outbound delivery fail closed. After local command handling, the
...
The Workflow Operator is deliberately separated from Mission data collection, while
sharing the canonical Core runtime. Only
an `APPROVAL_READY` plan can be proposed with `/workflow <riferimento-missione>` and a
one-use confirmation button. Confirmation calls `ADMIT_WORKFLOW_SPECIFICATION` for the
exact immutable `core-v1-content-direction@1.0.0` declaration and creates one durable,
preparation-only instance with one `content-direction` step. Telegram does not build or
persist an alternate definition. The instance cannot invoke the Agent Runtime until
Fabio approval, Quality Guardian, and Operator-Safety/Risk Guardian evidence has been
recorded through the controlled Core boundaries. `/report <riferimento-missione>` returns the durable
status, next checkpoint, bounded risks, and external-action statement; `/workflows`
shows the available flow. `/productions` shows the private, durable Metodo Veloce queue
and `/production <id>` opens one record. When it is waiting for Fabio, the detail view
offers one bound, one-use `Approva per calendario` confirmation. Confirmation records
only the exact-version internal approval: it does not select a date, publish, send a
